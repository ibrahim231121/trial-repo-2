...
const SortableList = SortableContainer(({orderColumn, headCells, chkStyle, onReOrderChange, dragHideState}: SortableListProps) => {
      ...
      <ul className="columnUlList" >
        {orderColumn.map((colIdx: any, index: number) => (
          ...
          <React.Fragment key={colIdx}>
            {(headCells[colIdx].keyCol === false || headCells[colIdx].keyCol === undefined) ? 
              <>
                <SortableItem 
                  key={colIdx} 
                  index={index}
                  value={
                      ...
                      <FormControlLabel
                        ...
                        control={<CRXCheckBox className="customizeCheckBox" checked={headCells[colIdx].visible} onChange={(e) => handleCheckChange(e,colIdx)} />}
