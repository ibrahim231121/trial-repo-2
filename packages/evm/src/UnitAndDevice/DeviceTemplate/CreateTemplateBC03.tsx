...
const CreateTemplate = (props:any) => {
  ...
  React.useEffect(() => {
   ...
   const loadData = async () => {
    ...
    const unitDataResponse = await fetch(
      `http://127.0.0.1:8083/ConfigurationTemplates/GetTemplateConfigurationTemplate?configurationTemplateName=${historyState.id}`,
      requestOptions
    );
  ...
  const handleSave = (values: any, resetForm: any) => {
    ...
    if (editCase == false)
    {
      ...
      const url = `http://127.0.0.1:8083/ConfigurationTemplates`;
