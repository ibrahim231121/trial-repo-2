...
const CustomizedMultiSelectForFormik = (props: any) => {
  ...
  const [valueArray, setValueArray] = useState<any>(Array.isArray(value) ? value : value.split(','));
    ...
    <>
      <CRXMultiSelectBoxLight
        ...
        onChange={(e: any, value: any[]) => {
          ...
          let allOptionsSelected = valueArrayTemp.filter((x: any) => x.toLowerCase() !== "all").length == options.filter((x: any) => x.label.toLowerCase() !== "all").length;
          let isAllSelected = valueArrayTemp.some((x: any) => x.toLowerCase() == "all");
          if (allSelected && isAllSelected == false) { //All selected removed
            setAllSelected(false);
            valueArrayTemp = [];
          }
          else if (allSelected && allOptionsSelected == false) { //Some option from all selected removed
            setAllSelected(false);
            valueArrayTemp = valueArrayTemp.filter((x: any) => x.toLowerCase() !== "all");
          }
          else if (allSelected == false && allOptionsSelected) { //All option selected individually
            setAllSelected(true);
            valueArrayTemp = [...valueArrayTemp, "All"];
          }
          else if (isAllSelected) { // All selected
            setAllSelected(true);
            valueArrayTemp = options.map((x: any) => x.value);
          }
          else { // All selected option wasnt involved
            valueArrayTemp = valueArrayTemp;
          }
          setValueArray([...valueArrayTemp]);
          setFieldValue(name, valueArrayTemp);
        }}
      />
    </>
  )
                    ...
                    render={(msg) => (
                      <>
                        <div className="UiLabelTextboxError">
                          <i className="fas fa-exclamation-circle"></i>  {(formObj.label ?? "") + " " + msg}
                        </div>
                      </>
                    ...
                    <ErrorMessage
                      ...
                      render={(msg) => (
                        <div className={formObj.belowFieldText ? `UiNumberSelectorError checkHelperTextPresent ` : `UiNumberSelectorError`}>
                          <i className="fas fa-exclamation-circle numberIconCircle"></i> {(formObj.label ?? "") + " " + msg}
                        </div>
                      )}
                    />
                  </div>
