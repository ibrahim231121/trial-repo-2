...
export const getAccessAndRefreshTokenAsync: any = createAsyncThunk('accessAndRefreshToken', async () => {
    ...
    if(accessToken)
    {
        ...
        if ( (tokenexpiry-300) < currentData && tokenexpiry > 0)
        {
            return await AuthenticationAgent.getAccessAndRefreshToken('/Authentication/GetAccessToken'+`?refreshToken=${localStorage.getItem('refreshToken')}`)
