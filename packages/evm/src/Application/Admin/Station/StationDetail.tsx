...
import { CRXMultiSelectBoxLight, CrxAccordion, CRXAlert, CRXModalDialog, GoogleMap, CRXButton, CRXRows, CRXColumn, CRXConfirmDialog } from '@cb/shared';
import './station.scss'
...
const StationDetail: React.FC = () => {
  ...
  const countryStatesResponse: any = useSelector((state: RootState) => state.stationReducer.countryStates);
  const [countryAutoCompleteOptions, setCountryAutoCompleteOptions] = React.useState<AutoCompleteOptionType[]>([]);
  const [statesAutoCompleteOptions, setStatesAutoCompleteOptions] = React.useState<AutoCompleteOptionType[]>([]);
  ...
  const [countryAutoCompleteValue, setCountryAutoCompleteValue] = React.useState<AutoCompleteOptionType[]>([]);
  const [stateAutoCompleteValue, setStateAutoCompleteValue] = React.useState<AutoCompleteOptionType[]>([]);
  ...
  const countryAutoCompleteonChange = (_: React.SyntheticEvent, val: AutoCompleteOptionType[], setFieldValue: any, reason: any) => {
  ...
  const stateAutoCompleteOnChange = (_: React.SyntheticEvent, value: AutoCompleteOptionType[], setFieldValue: any) => {
  ...
  const navigateToStations = () => {
    history.push(urlList.filter((item: any) => item.name === urlNames.adminStation)[0].url);
  }

  const onSubmit = (values: StationFormType, actions: FormikHelpers<StationFormType>) => {
    const body = {
      name: values.Name,
      address: {
  ...
  return (
    <>
      <Formik enableReinitialize={true} initialValues={stationPayload} onSubmit={onSubmit}>
        {({ setFieldValue, values }) => (

          <>
            <Form>
              <div className='ManageStation'>
                {success && <CRXAlert message='Success: You have saved the Station' alertType='toast' open={true} />}
                {error && (
                  <CRXAlert className='' message={errorResponseMessage} type='error' alertType='inline' open={true} />
                )}
                <CrxAccordion
                  title='Accordion one'
                  id='accorIdx1'
                  ...
                  <>
                    <div className='stationForm'>
                      ...
                      <div className='colstation'>
                        ...
                        <CRXMultiSelectBoxLight
                          ...
                          options={filterOptionsSource(countryAutoCompleteOptions)}
                          onChange={(e: React.SyntheticEvent, option: AutoCompleteOptionType[], reason: any) =>
                            countryAutoCompleteonChange(e, option, setFieldValue, reason)
                          }
                          CheckBox={true}
                          checkSign={false}
                          required={true}
                      ...
                      <div className='colstation'>
                        <label htmlFor='stateMultiSelect'>States</label>
                        <CRXMultiSelectBoxLight
                          key={reset}
                          id='stateMultiSelect'
                          multiple={false}
                          value={stateAutoCompleteValue}
                          onChange={(e: React.SyntheticEvent, option: AutoCompleteOptionType[]) =>
                            stateAutoCompleteOnChange(e, option, setFieldValue)
                          }
                    ...
                    <div className='stationForm'>
                      ...
                      <Field id='name' name='Name' placeholder='Station Name' />

                      <label htmlFor='street'>Adress</label>
                      <Field id='street' name='StreetAddress' placeholder='Address' />
                      ...
                      <Field id='phone' name='PhoneNumber' placeholder='Phone' />

                      <label htmlFor='latitude'>Latitude</label>
                      <Field id='latitude' name='Location.latitude' placeholder='Latitude' />

                      <label htmlFor='longitude'>Longitude</label>
                      <Field id='longitude' name='Location.longitude' placeholder='Longitude' />
                    </div>
                    <CRXButton className='groupInfoTabButtons' onClick={() => setModal(true)}>
                      Location
                    </CRXButton>
                  </>
                </CrxAccordion>
                <CRXButton type='submit' variant='contained' className='groupInfoTabButtons'>
                  Save
                </CRXButton>
                <CRXButton
                  ...
                  <GoogleMap
                    ...
                    {!isAddCase ?
                      ...
                      values.Location.latitude == 0 && values.Location.longitude == 0 ?
                        undefined : { lat: values.Location.latitude, long: values.Location.longitude }
                    }
